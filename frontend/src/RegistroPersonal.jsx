...
const RegistroPersonal = ({ onBack, onGoInicio, onGoInicioUsuario }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Aquí iría la lógica de envío. Por ahora solo consola.
    console.log("Registro personal:", form);
  };
    ...
    <div className="registroPersonal">
      ...
      <section className="registroPersonal__form-wrapper">
        <form className="registroPersonal__form" onSubmit={handleSubmit}>
          ...
          <input id="rp-nombre" name="nombre" value={form.nombre} onChange={handleChange} placeholder="" />

          <label className="registroPersonal__label" htmlFor="rp-email">Email</label>
          <input id="rp-email" name="email" value={form.email} onChange={handleChange} placeholder="ejemplo@gmail.com" />

          <label className="registroPersonal__label" htmlFor="rp-password">Contraseña</label>
          <input id="rp-password" name="password" type="password" value={form.password} onChange={handleChange} />

          <label className="registroPersonal__label" htmlFor="rp-password2">Confirmar contraseña</label>
          <input id="rp-password2" name="password2" type="password" value={form.password2} onChange={handleChange} />

          <button type="submit" className="registroPersonal__submit" onClick={() => onGoInicioUsuario && onGoInicioUsuario()}>Ingresar</button>
        </form>
      </section>
